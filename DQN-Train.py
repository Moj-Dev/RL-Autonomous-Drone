...
if train:
    ...
    log_filename = 'log10-d-dqn-ctd.json'.format(args.env_name)
    ...
    dqn.save_weights('log10_weights-ctd.h5f'.format(args.env_name), overwrite=True)


else:
    print("test")
    dqn.load_weights('log10_weights.h5f'.format(args.env_name))
    dqn.test(env, nb_episodes=51, visualize=False)
